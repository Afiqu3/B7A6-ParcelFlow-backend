...
const approveRider = async (
    ...
    if (existingRider.isDeleted) {
        throw new AppError(
            httpStatus.GONE,
            "Rider Application Has Been Deleted",
        );
    }
    ...
    const randomRiderPassword = isApproved
        ? generateRandomPassword()
        : undefined;

    if (config.node_env === "development" && randomRiderPassword) {
        console.log(`[dev] Random Password plain text: ${randomRiderPassword}`);
    }

    const hashedPassword = randomRiderPassword
        ? await bcrypt.hash(
              randomRiderPassword,
              Number(config.bcrypt_salt_rounds),
          )
    ...
    const templateData = {
        ...
        password: isApproved ? randomRiderPassword : undefined,
    };
        ...
        data: {
            ...
            riderProfile: {
                update: {
                    ...
                    address:
                        payload.address ?? existingRider.riderProfile?.address,
                },
            },
        },
        include: {
